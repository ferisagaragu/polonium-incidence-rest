define([
    ...
    Handlebars.registerHelper('generateFields', function(url, ref) {
        var data = url.split('/:');
        var out = '';
        ...
        if (data.length > 1) {
            for (let dataKey in data) {
                if (dataKey != 0) {
                    out +=
                    '<div class="input-group col-md-12 url-data">' +
                        '<input name="' + data[dataKey] + '" type="text" class="form-control text-param" placeholder="your ' + data[dataKey] + '" spellcheck="false" />' +
                        '<span class="input-group-addon">' + data[dataKey] + '</span>' +
                    '</div>'
                }
            }
        }
    ...
    Handlebars.registerHelper('typeAndExample', function(type) {
        var example = type.split('/')[0];

        switch (example.toLowerCase()) {
            case 'string': return new Handlebars.SafeString('<label style="color: #448AFF">' + example + '</label>');
            case 'number': return new Handlebars.SafeString('<label style="color: #E53935">' + example + '</label>');
            case 'boolean': return new Handlebars.SafeString('<label style="color: #6200EA">' + example + '</label>');
            default: return new Handlebars.SafeString('<label>' + example + '</label>');
        }
    });

    Handlebars.registerHelper('inputTypeParam', function(type) {
        var example = type.split('/')[0];

        if (example.toLowerCase() == 'number') {
            return 'number';
        } else {
            return 'text';
        }
    ...
    Handlebars.registerHelper('inputTypeParamValue', function(type) {
        var data = type.split('/');
        var out = '';

        if (data.length > 1) {
            out = data[1].replace('[space]',' ');
        }

        return out;
    });

    Handlebars.registerHelper('removeOrder', function(text) {
        var out = text.split('>');
        if (out.length > 1) {
            return out[1];
        }
    ...
    Handlebars.registerHelper('convertTitle', function(text) {
